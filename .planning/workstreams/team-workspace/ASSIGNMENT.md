...
|-------|-------|------|-----------|---------|
| **32** | W1 | **Embed Parrot into the SMS/phone pane** — Parrot is a *separate* dialer/phone+SMS product (built by our own dev); embed its **internjobs tenant** via `<iframe>` + OIDC SSO (reuse the Mattermost `/oidc/*` bridge — Parrot = second OIDC client). **Do NOT build a dialer or reinvent UX.** Completes the tripod (Mail→Agent-Inbox phase 30, Chat→Mattermost phase 31 already shipped). | Parrot internjobs tenant embedded + silently SSO'd in the pane; no built dialer | **`PARROT-EMBED-SPEC.md`** (this dir) |

...
- [ ] **Phase 15: Mattermost OIDC SSO activation** — Activate the Phase 10 Wave 2b OIDC bridge code that's already shipped at `apps/parrot/workers/routes/oidc.ts` + WorkspaceDO `oidc_codes`/`oidc_tokens` tables. Configure Mattermost's `GitLabSettings.*` (CF auth provider slot) via mmctl to point at Parrot's `/oidc/authorize|token|userinfo` endpoints. After this lands, employees who sign into Parrot via Clerk are auto-signed-in to the Mattermost iframe — no separate Mattermost login prompt. (Absorbed from v1.3 candidates 2026-05-19 per "finish everything in this milestone" direction.)

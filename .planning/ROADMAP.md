...
**Status:** 🚧 v1.2 — Two-Sided Agent MVP (FULLY code-complete 2026-05-19 incl. Phase 11 Daily.co; awaiting user-action gates: DAILY_API_KEY + INTEG-01 prod smoke + browser visual verification)
...
- [△] **Phase 14: Parrot Knowledge Graph (FalkorDB extension)** — Code shipped 2026-05-19 (3 waves: graph.ts helper + Phase 12 wiring + smoke/healthz). **Runtime-blocked on infra bridge:** `falkordb` npm package crashes Cloudflare Workers runtime with `BigInt is not a function`. Dynamic-imported in graph.ts so Worker boots cleanly; all exports fail-soft to no-op when import fails. `/healthz.graph_ready=false` in production until activation. Two activation paths: (a) thin Fly REST proxy in front of FalkorDB, OR (b) Workers-native RESP3 client via cloudflare:sockets + publicly-reachable FalkorDB endpoint. Both are v1.3 hardening. Code path is verified type-clean + deploys cleanly with graceful degradation.
...
- [x] **Phase 16: Ridhi Admin Invite UX (capability toggles)** — Shipped 2026-05-19 (2 waves). Backend (Wave 1): `/api/admin/employees` POST extended with optional phoneNumber (E.164 validated) + featureFlags; `createClerkUser` supports phone-OTP enrollment (mutually exclusive with email_address); welcome email rewritten in operator's voice with mission + phone-OTP login instructions; GET + PATCH `/api/admin/employees/:id/flags` via KV (`employee:{clerk_user_id}:flags`) with read-modify-write merge. Frontend (Wave 2): `/admin` employee directory with capability pills + inline toggle editor + disable action; `/admin/invite` rewritten with FN/LN/personalEmail/phone + 6 capability toggles (email/chat/meetings/phone/sms/campaigns) defaulting all-on. Backward compatible — legacy `{name, personalEmail}` invites still return 201. Worker version `44c33cb1` live.

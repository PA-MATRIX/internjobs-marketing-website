...
live_progress_2026_07_17:
  ...
  still_open: "NONE — superseded by end_to_end_uat_2026_07_17 above. At the time this key was written no flagged mail had yet arrived; the operator then sent one and it landed in Spam and was recovered via Trust sender, closing every item."
human_verification:
  - test: "Send a Lakera-flagged (prompt-injection) inbound email from a NON-member, NON-trusted external sender to a real employee mailbox"
    ...
    status_2026_07_17: "CLOSED — PASSED. Operator sent a prompt-injection email from a personal address to a live internjobs.ai mailbox; it landed in the Spam folder (quarantined + persisted, not dropped). The folder_id='spam' write on a real DO is now observed in production."
  ...
  - test: "Load /inbox?folder=spam in a real browser against a deployed Worker, click a quarantined message, click Trust sender, and read the resulting toast"
    ...
    status_2026_07_17: "CLOSED — PASSED. The Spam sidebar item renders live, the quarantined message was visible and openable, and clicking Trust sender moved it into the Inbox (operator, 2026-07-17). Full click-path observed in production."
---
...
The phase's own executors were honest about two structural blind spots that no test in this repository could close: (1) the DO is fully mocked everywhere, so the storage layer and migration self-application had never run against a real SQLite-backed Durable Object; (2) the local dev server cannot boot (a pre-existing, unrelated issue), so the Spam UI had never been seen rendering by a human or browser. Neither was a code gap — they were coverage-depth limitations that only a live deploy plus operator UAT could close.

**Both are now closed (2026-07-17).** The Worker was deployed (version `3347a2c7`) and boot-checked live, the operator signed in — proving migration 10 self-applied, since `applyMigrations()` runs in the DO constructor and a faulty migration would have errored the mailbox rather than rendering it — and then sent a real prompt-injection email from a personal (non-member, non-trusted, non-skip-listed) address. It **landed in the Spam folder**, was visible and openable, and **Trust sender moved it back to the Inbox**. The quarantine write, the render, and the recovery path are therefore observed in production, not inferred from code reading. Status is `passed`. See `end_to_end_uat_2026_07_17` in the frontmatter.

One follow-up remains (not a gap in this phase's goal): the operator's test sender is now permanently in `trusted_senders`, and there is no revoke UI. A confirm dialog was added (`558dfd6`) to guard the misclick; a trusted-senders review/revoke surface is tracked as future work.

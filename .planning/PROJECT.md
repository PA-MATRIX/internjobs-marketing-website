...
| Workers AI via proxy Worker (not OpenAI direct, not AI Gateway REST) for v1.2 LLM path | Zero new user-managed CF API tokens (the Worker's `env.AI` binding is intrinsic to the Worker — wrangler OAuth is enough to deploy); native Worker AI runtime is the fastest path with no extra hop; embeddings use `@cf/baai/bge-base-en-v1.5` (768-dim) and chat uses `@cf/meta/llama-3.1-8b-instruct`; optional AI Gateway routing via `{gateway:{id:'internjobs-ai'}}` when the gateway is created (best-effort — falls back to direct Workers AI when missing, CF error code 2001) | ✓ Shipped 2026-05-16 — `internjobs-ai-proxy.rentalaraj.workers.dev` |

---
*Last updated: 2026-05-16 — Workers AI swap (OpenAI → Cloudflare Workers AI via the internjobs-ai-proxy Worker; embedding column `vector(1536)` → `vector(768)` via migration 0005; openai npm dep removed; /healthz drops openaiKeyPresent and adds aiProxyReady). Earlier 2026-05-16 update: STORAGE-01 + EMAIL-03 scope-add (private R2 bucket scaffold with per-entity folders + per-conversation Reply-To aliases for deterministic threading; modeled on SuperIntelligence patterns). Prior 2026-05-16 update: Resend → Cloudflare Email Service swap for EMAIL-02 outbound (already on CF DNS, one less vendor; CF "Agent Mail" public beta 2026-04-17). 2026-05-15 update: v1.1 completion and v1.2 scope revision (Telnyx held for v1.3, Spectrum stays active behind `SmsProvider` seam).*

...
- **S1 — Startups web app go-live** (team-workspace): execute `PHASE-28.5-DEFERRED-OPS.md` (DNS, Email Routing domain verification, `STARTUPS_CLERK_*` secrets → Infisical, Clerk webhook) to bring `startups.internjobs.ai` + per-startup agent email live.
- **S2 — Telnyx SMS + Voice AI go-live** (team-workspace): execute `PHASE-29-DEFERRED-OPS.md` (Telnyx signup + toll-free number + BRN + API key + Voice AI portal + R2 + KV + cron) to bring startup SMS + voice-intake onboarding live.
- **S3 — First live pilot install** (team-workspace): STARTUP-PILOT-LIVE-01 — real founder installs MCP (or web onboard) and completes `me()` + `post_role` + `search` + `reply`; evidence in `PILOT-EVIDENCE.md`.
- **S4 — Lakera fail-open + tier confirm** (team-workspace): LAKERA-VERIFY-LIVE-03 (deferred fail-open test) + tier/quota confirmation for pilot volume (`infra/LAKERA-PRICING.md`).
...
**Track S — Startup pilot go-live (team-workspace):**

- [ ] **S1 STARTUP-28.5-OPS-LIVE**: Execute `PHASE-28.5-DEFERRED-OPS.md` (12 steps: DNS, Email Routing domain verification, `wrangler secret put STARTUPS_CLERK_*` → Infisical, Clerk webhook) to bring `startups.internjobs.ai` + per-startup agent email live. **Team: team-workspace**
- [ ] **S2 STARTUP-29-OPS-LIVE**: Execute `PHASE-29-DEFERRED-OPS.md` (22 steps: Telnyx signup + toll-free number + BRN + API key + Voice AI portal config + R2 bucket + KV namespace + cron deploy) to bring startup SMS + voice-intake onboarding live. **Team: team-workspace**
- [ ] **S3 STARTUP-PILOT-LIVE-01**: First real founder installs MCP (or web-onboards) and completes `me()` + `execute('post_role')` + `search('candidates')` + `execute('reply_to_candidate')`; record tool-call evidence in `PILOT-EVIDENCE.md`. **Team: team-workspace** (Ridhi concierge)
- [ ] **S4 LAKERA-VERIFY-LIVE-03 + TIER-CONFIRM**: Run the deferred Lakera fail-open test (if pilot-critical) + confirm tier/quota sufficient for pilot volume (`infra/LAKERA-PRICING.md`). **Team: team-workspace**

...
export async function renderPairing({ student, pairing, config }) {
  ...
  const body = `Join InternJobs.ai with ${pairing.code}`;
  ...
  return `
    <section class="pair-grid">
      <div class="panel">
        ...
        <h1>Send the code. Get the first text.</h1>
        <p class="lede">Scan this with your phone or send the code manually. The code expires in 15 minutes and can be regenerated.</p>
      ...
      <div class="panel dark">
        ...
        <p class="lede">Text this code to <strong>${escapeHtml(config.photon.fromNumber || "the InternJobs.ai number once configured")}</strong>.</p>

...
function MeetingsSecondaryNav() {
	return (
		<nav className="py-3">
			...
			<SecondaryNavItem href="/meetings" active label="Today" icon="📅" />
			<SecondaryNavItem href="/meetings" label="Upcoming" icon="•" />
			<SecondaryNavItem href="/meetings" label="Recordings" icon="🎬" />
			<p className="px-5 py-1 mt-3 text-[10px] font-semibold uppercase tracking-wider text-slate-400">
				Quick start
			</p>
			<SecondaryNavItem href="/meetings" label="Start instant" icon="▶" />
			<SecondaryNavItem href="/meetings" label="Schedule" icon="+" />
		</nav>
	);
}

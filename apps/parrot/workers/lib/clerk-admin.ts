...
// with a phone-number login identifier.
//
...
export interface ClerkUserCreateInput {
	/**
	 * E.164 phone number (e.g. "+12125551234"). The Parrot employee Clerk
	 * app is phone-OTP only, so the user is created with a phone_number
	 * identifier and NO email_address. Workspace email is internal Parrot
	 * state, not a login credential.
	 */
	phoneNumber: string;
	firstName: string;
	lastName: string;
...
export async function createClerkUser(
	...
	if (!E164_REGEX.test(input.phoneNumber)) {
		throw new Error(
